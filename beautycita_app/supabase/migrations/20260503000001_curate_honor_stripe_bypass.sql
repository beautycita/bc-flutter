-- =============================================================================
-- 20260503000001 — curate_candidates honors stripe_bypass
-- =============================================================================
-- Drift between memory `stripe_required_for_search.md` and deployed
-- `curate_candidates`. The memory documents the search gate as
--   (stripe_charges + stripe_payouts) OR stripe_bypass
-- but the deployed function body has
--   AND b.stripe_charges_enabled = true
-- with no bypass branch and no payouts check. Setting stripe_bypass=true on
-- a salon does nothing for search. Found 2026-05-03 while making
-- Salon Studio Kriket searchable for BC's end-to-end test.
--
-- Fix: replace the literal-true gate with the OR-bypass form. CREATE OR
-- REPLACE FUNCTION preserves grants; no DROP.
-- =============================================================================
...
AS $function$
  WITH service_match AS (
    ...
    WHERE s.service_type = p_service_type
      ...
      -- Stripe gate: pair must be enabled OR explicit bypass.
      -- Authoritative source: memory/stripe_required_for_search.md.
      AND (
        (b.stripe_charges_enabled AND b.stripe_payouts_enabled)
        OR COALESCE(b.stripe_bypass, false)
      )
...
COMMENT ON FUNCTION public.curate_candidates(
  text, double precision, double precision, integer,
  timestamp with time zone, timestamp with time zone, uuid
) IS
  'Search-gate truth: salon must have (stripe_charges + stripe_payouts) OR stripe_bypass. Other gates: is_active, is_verified, onboarding_complete, banking_complete, staff active+accept_online_booking+owner|stylist.';
