-- Fix count_eligible_recipients for recipient_table='businesses'.
--
-- The previous version joined public.profiles for the owner's email, but
-- public.profiles has no email column on prod (email lives on auth.users
-- and on businesses.email). Triggered Postgres 42703 ("column p.email
-- does not exist") whenever an admin tried to count eligibility for any
-- registered-salon outreach.
--
-- Salons have their own contact email on businesses.email — that is the
-- authoritative recipient channel for outreach. Drop the profiles join
-- and read businesses.email directly.
