-- =============================================================================
-- Down: rollback cash trust tier platform-wide.
-- =============================================================================
-- Restores create_booking_with_financials to the body installed by 000000.
-- Drops trust-tier schema, RPCs, cron, trigger, templates.
-- =============================================================================
...
DROP FUNCTION IF EXISTS public._trg_cash_recompute_on_debt_clear();

-- ── Restore create_booking_with_financials to the 000000 / 000001 body ─────
-- This is the body that 20260426000001 installed (hard-block on cita_express
-- cash). The user can re-apply 000001 separately if rolling back further is
-- desired; this restore puts them back at the "no trust tier" state.
...
DELETE FROM notification_templates
 WHERE event_type IN ('cash_activated','cash_suspended','cash_reactivated')
   AND channel = 'email' AND recipient_type = 'salon';

-- ── Functions ──────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.sweep_cash_eligibility();
DROP FUNCTION IF EXISTS public.is_cash_eligible(uuid);
DROP FUNCTION IF EXISTS public.compute_cash_eligibility(uuid);
DROP FUNCTION IF EXISTS public.count_cash_eligible_tx(uuid);

-- ── app_config ─────────────────────────────────────────────────────────────
DELETE FROM app_config WHERE key IN ('cash_trust_min_tx', 'cash_block_tax_debt_threshold');

-- ── State log + business columns ───────────────────────────────────────────
DROP TABLE IF EXISTS businesses_cash_state_log;
ALTER TABLE businesses
  DROP COLUMN IF EXISTS cash_eligible_at,
  DROP COLUMN IF EXISTS cash_blocked_at,
  DROP COLUMN IF EXISTS cash_block_reason,
  DROP COLUMN IF EXISTS cash_tx_count_cached;
