-- =============================================================================
-- Cash trust gate: only fires for on-network bookings
-- =============================================================================
-- The trust gate in 000002 raised on ANY cash_direct booking when the salon
-- wasn't cash_eligible. That's wrong for off-network bookings:
--   * salon_direct is OFF-network → BC has no SAT obligation → no tax_obligation
--     debt is ever created → cash should always be allowed.
--   * Trust only matters for on-network bookings where BC takes tax retention.
--
-- Fix: gate `cash_direct` on `is_cash_eligible(business_id)` ONLY when
-- booking_source is in (bc_marketplace, invite_link, cita_express).
-- =============================================================================
...
BEGIN
  ...
  -- TRUST GATE: cash_direct on ON-NETWORK bookings requires earned trust.
  -- Off-network (salon_direct) bypasses the gate — the salon handles its own
  -- tax retention there, BC has no SAT obligation, no tax_obligation debt is
  -- created, so the trust mechanism is irrelevant.
...
COMMENT ON FUNCTION public.create_booking_with_financials(uuid, uuid, text, text, text, timestamptz, timestamptz, numeric, text, text, text, uuid, text, text, numeric) IS
  'Trust-tier cash gate: cash_direct on ON-NETWORK bookings (bc_marketplace/invite_link/cita_express) requires is_cash_eligible(business_id). Off-network bookings (salon_direct) bypass the gate. is_test businesses skip ledger writes.';
