-- =============================================================================
-- create_booking_with_financials: fix saldo idempotency-key collision
-- =============================================================================
-- Bug found via bughunter booking-create-and-cancel flow: the auto-generated
-- saldo idempotency key used p_starts_at which is stable across runs on the
-- same day → second/third bookings with the same start time were deduped at
-- the saldo layer (silent skip) while the appointment was still inserted.
-- Result: the salon-side ledger drifted from profiles.saldo (the test caught
-- this exactly).
--
-- Fix: pre-generate the booking_id, use it in the saldo idem key. Each
-- appointment is uniquely keyed; replays from the same client only collide
-- if they pass the same p_idempotency_key (their explicit choice).
-- =============================================================================
...
BEGIN
  ...
  IF p_payment_method = 'saldo' THEN
    ...
    -- Use the pre-generated booking_id for the saldo idem key. Each booking
    -- gets a unique key — no false dedup across same-day calls. If the
    -- caller passed an explicit p_idempotency_key, derive from it (so the
    -- caller's explicit replay protection still works).
    v_saldo_idem := COALESCE(p_idempotency_key, v_booking_id::text) || ':saldo';
