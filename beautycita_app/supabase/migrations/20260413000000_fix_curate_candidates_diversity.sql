-- =============================================================================
-- Migration: 20260413000000_fix_curate_candidates_diversity.sql
-- Description: Fix curate_candidates RPC to ensure business diversity.
--              Old: ORDER BY distance LIMIT 50 — closest salon ate all slots.
--              New: ROW_NUMBER() PARTITION BY staff_id, max 5 slots per staff,
--              guaranteeing multiple businesses appear in results.
-- =============================================================================
...
RETURNS TABLE(
  ...
  cancellation_hours integer, deposit_required boolean,
  auto_confirm boolean, accept_walkins boolean,
...
AS $$
  ...
  -- Limit slots per staff member to ensure business diversity.
  -- Without this, the closest salon's staff fill all 50 slots.
  ranked_slots AS (
    SELECT
      sm.*,
      avail.slot_start,
