-- Migration: search_discovered_salons v2
-- Extends the RPC with geo radius search + RP assignment filters.
-- Must DROP old signature first (param list changed).
...
BEGIN
  ...
  -- Determine if geo search is active
  has_geo := p_pin_lat IS NOT NULL AND p_pin_lng IS NOT NULL AND p_radius_km IS NOT NULL;
  ...
  SELECT
    ...
    (
      ...
      -- Geo proximity bonus: closer = higher score
      + CASE
          WHEN has_geo AND d.latitude IS NOT NULL AND d.longitude IS NOT NULL THEN
            GREATEST(0, 10.0 - (
              (6371.0 * acos(LEAST(1.0,
                cos(radians(p_pin_lat)) * cos(radians(d.latitude)) * cos(radians(d.longitude) - radians(p_pin_lng))
    ...
    d.latitude,
    d.longitude,
    (SELECT p.full_name FROM profiles p WHERE p.id = d.assigned_rp_id) AS rp_name
...
-- Grant execute to authenticated users (new full signature)
GRANT EXECUTE ON FUNCTION public.search_discovered_salons(text, text[], text, boolean, boolean, text, int, int, uuid, text, double precision, double precision, double precision) TO authenticated;
