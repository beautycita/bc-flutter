-- Down: revert is_test gates on cancel_booking + purchase_product_with_saldo.
-- Restores the prior versions from 20260423000002 and 20260424000000.
...
BEGIN
  ...
  IF v_booking.payment_status IN ('unpaid', 'expired', 'failed') THEN
    v_refund_amount := 0; v_bc_commission := 0; v_payment_status := v_booking.payment_status;
  ELSIF p_cancelled_by = 'business' THEN
    v_refund_amount := v_booking.price; v_payment_status := 'refunded_to_saldo';
  ELSIF v_hours_until >= v_free_cancel_window_hours THEN
    v_refund_amount := v_booking.price - v_bc_commission; v_payment_status := 'refunded_to_saldo';
  ...
  IF v_refund_amount > 0 AND v_booking.business_id IS NOT NULL THEN
    INSERT INTO salon_debts (business_id, appointment_id, original_amount,
      remaining_amount, reason, source, created_at)
    VALUES (v_booking.business_id, v_booking.id, v_refund_amount, v_refund_amount,
  ...
  IF v_bc_commission > 0 AND v_booking.business_id IS NOT NULL THEN
    INSERT INTO commission_records (business_id, appointment_id, amount, rate, source,
      period_month, period_year, status, created_at)
    VALUES (v_booking.business_id, v_booking.id, v_bc_commission, v_commission_rate,
      CASE WHEN p_cancelled_by = 'business' THEN 'salon_cancellation' ELSE 'customer_cancellation' END,
      EXTRACT(MONTH FROM now())::int, EXTRACT(YEAR FROM now())::int, 'collected', now())
    ON CONFLICT DO NOTHING;
  END IF;
  UPDATE appointments
...
-- Down for purchase_product_with_saldo intentionally not provided in full —
-- the prior body lives in 20260424000000_purchase_product_idempotency_key.sql
-- and would be re-applied by re-running that migration if needed.
