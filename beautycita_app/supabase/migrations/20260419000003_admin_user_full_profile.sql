-- ─────────────────────────────────────────────────────────────────────────────
-- admin_get_user_full_profile — one-shot aggregator for the admin user view.
-- ─────────────────────────────────────────────────────────────────────────────
-- The old web detail panel (user_detail_panel.dart) rendered about 12 bare
-- fields from the profiles row. BC's feedback: "useless and not visible".
-- This RPC returns everything the admin dashboard could reasonably want on
-- a single user in one jsonb payload, so the UI can stop doing 10 queries
-- per open.
--
-- Security: admin or superadmin only (gated via is_admin()). Logs access
-- to admin_trait_access_log for the behavioral section, same as
-- admin_view_user_traits does.
--
-- Freshness: computes trait scores on-demand (PERFORM compute_user_traits)
-- so the admin always sees current state rather than whatever the last
-- nightly sweep left behind.
...
COMMENT ON FUNCTION public.admin_get_user_full_profile(uuid) IS
  ...
  'Admin/superadmin only. Computes behavioral traits on-demand. '
