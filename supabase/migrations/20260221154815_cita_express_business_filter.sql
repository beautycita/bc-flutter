-- =============================================================================
-- Migration: 20260221154815_cita_express_business_filter.sql
-- Description: Add optional p_business_id parameter to curate_candidates RPC.
--              When provided, filters to that specific business (for Cita Express
--              walk-in flow) and skips the PostGIS radius check.
-- =============================================================================
...
as $$
  with service_match as (
    ...
    where s.service_type = p_service_type
      ...
      and (
        (p_business_id is not null and b.id = p_business_id)
        or
        (p_business_id is null and ST_DWithin(
          b.location,
          ST_MakePoint(p_lng, p_lat)::geography,
          p_radius_meters
        ))
...
comment on function public.curate_candidates is
  ...
  'Joins businesses, services, staff_services, staff with PostGIS spatial '
  'filtering and find_available_slots for availability checking. '
  'Optional p_business_id locks results to a specific business (Cita Express).';
